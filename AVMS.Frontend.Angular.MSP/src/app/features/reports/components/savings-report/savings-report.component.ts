...
import { SavingsData, SavingsDataItem } from '../../models/savings-report-structure';
...
export class SavingsReportComponent implements OnInit, OnDestroy {

  data: SavingsDataItem[] = [];  
  breadcrumbs: BreadcrumbsItem[] = [
    { label: 'Home', url: '/' },
    { label: 'Reports: Savings', url: ''},
  ];
  displayedColumns: string[] = ['Time range', 'Savings'];
  accounts: Account[] = [];
  ...
  headers: TableHeader[] = [
    ...
    { description: 'Savings', sortable: true, sortOrder: 'asc' },
  ];
  dataSource = new MatTableDataSource<SavingsDataItem>();
  ...
  getMainGraphData(interval: number): void {
    ...
    let report$ = this.httpService.getMonthlySavingsReport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
    let days = 999;
    if (interval === 1 || interval === 2) {
      if(interval === 1){
        report$ = this.httpService.getSummarySavingsReport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
      }
      else if(interval === 2){
        report$ = this.httpService.getDailySavingsReport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
      }
      days = interval === 1 ? 7 : 31;
    }
  ...
  export(interval: number): void {
    let report$ = this.httpService.getMonthlySavingsReportExport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
    if(interval === 1){
      report$ = this.httpService.getSummarySavingsReportExport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
    }
    else if(interval === 2){
      report$ = this.httpService.getDailySavingsReportExport(+this.selectedTenant, +this.selectedSubscription, +this.selectedResourceGroup, +this.selectedProvider);
    }
  ...
  getSavedFilters() {
    ...
    this.httpService.getSavingsReportFilter(this.companyId).subscribe(res => {
  ...
  openSaveNewFilter(){
    ...
    this.addFilterRef.componentInstance.filter = "savings";
